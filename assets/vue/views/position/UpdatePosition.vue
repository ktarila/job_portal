        ...
        <div class="flex items-center justify-end p-6 border-t border-solid border-gray-300 rounded-b">
          <div>
            <button
              ...
              type="button"
              style="transition: all .15s ease"
              @click="getPosition(newPosition.id)"
            >
              Reset
            </button>
            <button
...
export default {
  ...
  methods: {
    ...
    setPosition(res){
      let position = {id: res.id, title: res.name, country: res.country, state: res.state, deadline: new Date(res.deadline), description: res.description}
      this.newPosition = position
      this.selectedCountryIndex = this.countries.findIndex(item => item.id === position.country.id);
      this.selectedStateIndex = this.countries[this.selectedCountryIndex].states.findIndex(item => item.id === position.state.id);
    },
    async submitForm(){
      ...
      if (isValid) {
        this.isLoading = true;
        let _this = this;
        PositionAPI.updatePosition(this.newPosition)
          .then(function(response) {
            _this.isLoading = false
            _this.showModal = false
            return response.data
          }).then(data => {
            let notification = {notification: data.name + " updated", type: "success"}
            store.dispatch('addNotification', notification)
            this.$router.push({ name: 'show-position', params: {id: this.newPosition.id} });
